...
export async function createMonitor(
  ...
  await db
    .prepare(
      'INSERT INTO monitors (id, name, url, interval_minutes, timeout_ms, alert_webhook) VALUES (?, ?, ?, ?, ?, ?)'
    )
    .bind(m.id, m.name, m.url, m.interval_minutes, m.timeout_ms, m.alert_webhook)
    .run();
}
